...
void splitProcs_LBCM(){
    ...
    MPI_Comm_size(col_comm , &colCommSize) ; 


	rowkey = world_rank % nsegments ; 
	MPI_Comm_split(MPI_COMM_WORLD , rowklass , rowkey , &row_comm);
    MPI_Comm_rank(row_comm , &myrowRank) ; 
    MPI_Comm_size(row_comm , &rowCommSize) ; 
...
//////////////////////diagonal major ordering /////////////////

/*void splitProcs_LBDM(){
...
//	int world_rank ; 
	...
	if(idiag > 0 ){
        ...
        //#pragma omp task depend (inout : totaldim) ; 

    	//	MPI_Allreduce(MPI_IN_PLACE , totaldim , 1 , MPI_INT , MPI_SUM , diag_comm) ; 


        #pragma omp task depend(out: amp) depend(in : distamp)
        {
        ...
        MPI_Iallgatherv(distamp , nblk * lvecdim , MPI_DOUBLE , amp ,  ilvecshare , ilvecdisp , MPI_DOUBLE , col_comm) ; 
    ...
    if(myrowRank == 0 ){
        /*for(i = 1 ; i < rowCommSize ; i++){

                #pragma omp task depend(inout : ampT)
	...
	MPI_Bcast(ampT ,nrow , MPI_INT , 0 , row_comm) ; 
...
/*    #pragma omp parallel for default(shared) private( rbase, cbase, j, k, l, r, c, xcoef, index)
    for(i = 0; i < nrowblks ; i++)
    {
        //tstart = omp_get_wtime();
...
//////////////////////////// Reduce the results along row communicators ////////////////////////
	...
	if(idiag > 0){
        ...
        #pragma omp task depend(inout : Hammp)
        {
...
//////////////////////////////////////////
    ...
    #pragma omp task depend task depend(in : HammpT) depend(out : distHamp)
    {
    ...
    MPI_Ireduce_scatter(HammpT , distHamp , nblk * ilvecshare[nsegments] , MPI_DOUBLE , MPI_SUM , col_comm) ; 
